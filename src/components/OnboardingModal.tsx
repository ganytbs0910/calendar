...
// First run: say what this calendar is for, then set up the one thing that
// makes its headline number real. Two pages, so the figure the app is named
// for is two taps away.
//
// It used to be six. Four of those pages described features to someone who had
// not yet seen the app, and three of the four were already explained a second
// time by a OneTimeHint sitting on the feature itself — colours on the event
// editor, the wage on the work-colour row, the income wall on its own screen.
// A hint that fires where the thing is beats a slide read before the thing
// exists, so the slides went and the hints stayed. The notification page is
// now one line on the setup page, next to the button that triggers the
// permission dialog.
//
// The setup page is the sleep-rhythm control itself rather than a slide about
// it. It used to be a separate Modal raised at the same time as this one, and
// iOS silently dropped the second presentation: a new user reached the
// calendar having never seen it, so the number this whole app is named for
// stayed unset. Folding it in here means there is only ever one modal, and the
// user leaves onboarding with the headline figure already working.
const OnboardingModal: React.FC<OnboardingModalProps> = ({visible, onClose}) => {
  const {t} = useTranslation();
  const {colors} = useTheme();
  const styles = useMemo(() => makeStyles(colors), [colors]);
  // A Modal fills the screen edge to edge and brings no safe-area padding of
  // its own, so "スキップ" sat under the status bar — on the very first screen a
  // new user sees, half hidden behind the clock and the battery.
      ...
      <View
        ...
        <ScrollView
          ...
          <View style={[styles.slide, {width: pagerWidth}]}>
            ...
            {/* The CTA below raises the notification dialog. Saying so here is
                what the dedicated notification slide used to do, minus a page. */}
            <Text style={styles.note}>{t('onbRhythmNotifyNote')}</Text>
          </View>
        </ScrollView>
...
const makeStyles = (colors: ThemeColors) => StyleSheet.create({
  ...
  note: {fontSize: 12, color: colors.textTertiary, textAlign: 'center', lineHeight: 18, marginTop: 4},
